...
class Timer
{
    ...
    int get()
    {
        return (int)(System.nanoTime() - tPoint);
    }
}
...
/*
        ...
        for (int i = 0; i < rankingSize;i++)
        {
            if (elements[i].szId.equals("0"))
                i1++;
            if (elements[i].szId.equals("1"))
                i2++;
            if (elements[i].szId.equals("2"))
                i3++;
        }
...
{
    KNearestNeighbors(String[] features, int size)
    {
        documents = new Dokument[size];
        //Feature and document count
        documentCount = size;
        ...
        for (int i = 0; i < size;i++)
        {
            documents[i] = new Dokument(featureCount);
        }

        //Init featureList
        featureList = new String[featureCount];

        //Init min max vectors
        minFeatures = new double[featureCount];
        maxFeatures = new double[featureCount];
    ...
    {
        double distance[] = new double[documentCount];

        double normalizedReq[] = new double[featureCount];
        ...
        for (int i = 0; i < featureCount;i++)
        {
            ...
            for (int i = 0; i < documentCount; i++)
            {
                if (!useNormalization)
                {
                    switch (type)
                    {
                        case EuklidischerAbstand: distance[i] = fnEuklidischerAbstand(documents[i].iFeatures,reqVector); break;
                        case ManhattanMetrik: distance[i] = fnManhattanMetrik(documents[i].iFeatures,reqVector); break;
                        case TschebyschewNorm: distance[i] = fnTschebyschewNorm(documents[i].iFeatures,reqVector); break;
                        case KosinusÄhnlichkeit: distance[i] = fnKosinusAhnlichkeit(documents[i].iFeatures,reqVector);
                    }
                }
                else
                {
                    switch (type)
                    {
                        case EuklidischerAbstand:
                            distance[i] = fnEuklidischerAbstand(documents[i].nFeatures, normalizedReq);
                            break;
                        case ManhattanMetrik:
                            distance[i] = fnManhattanMetrik(documents[i].nFeatures, normalizedReq);
                            break;
                        case TschebyschewNorm:
                            distance[i] = fnTschebyschewNorm(documents[i].nFeatures, normalizedReq);
                            break;
                        case KosinusÄhnlichkeit:
                            distance[i] = fnKosinusAhnlichkeit(documents[i].nFeatures, normalizedReq);
                    }
                }
            ...
            for (int i = 0; i < documentCount; i++)
            {
                ranking.add(documents[i].szClass,distance[i]);
            }
    ...
    String integerToFixedSizeString(double number, int desiredLength)
    {   //Since string has no append method we need string builder
        int integer = (int)(number);

        String numberAsString = String.valueOf(integer);
        StringBuilder sb = new StringBuilder();
        while(sb.length()+numberAsString.length()<desiredLength) {
            sb.append('0');
        }
        sb.append(integer);
        return sb.toString();
    }
    String integerToFixedSizeString(int number, int desiredLength)
    {   //Since string has no append method we need string builder
    ...
    //Return an value how often the string token is inside the source string
    int getTokenCount(String source, String token)
    {   //Probably already defined in some string utility class
        int lastIndex = 0, count = 0;
        while (true)
    ...
    boolean readDocumentFile(String documentPath)
    {
        ...
        try //Its not like its necessary on this point since we already test if the file exists
        {
            ...
            {   //Copy file line into string
                ...
                {
                    documents[currentLine].szId = szID;
                    documents[currentLine].szClass = szClass;
                    documents[currentLine].szText = szText;
    ...
    {   //Iterate through documents
        ...
        {  //Iterate through feature vector
           ...
           {   //get token count of the current document and feature
               documents[i].iFeatures[ii] = getTokenCount(documents[i].szText,featureList[ii]);
               //Check min max vectors -> update if necessary
               if (documents[i].iFeatures[ii] > documents[ii].maxValue)
                   maxFeatures[ii] = documents[i].iFeatures[ii];
               if (documents[i].iFeatures[ii] < minFeatures[ii])
                   minFeatures[ii] = documents[i].iFeatures[ii];
           }*/
            documents[i].scanDocumentForFeatures(featureList);
        }
        return true;
    }
    ...
    {   //Iterate through documents and call minMaxNormalize
        for (int i = 0; i < documentCount; i++)
        {
            documents[i].minMaxNormalize(minFeatures,maxFeatures);
        }
    }
    ...
    void outputDocumentData()
    {
        ...
        //System.out.println("ID  |  CLASS  | A  | B  | C  | D  | E  | F  | Text  ");
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            out =  documents[i].szId + "  |  " + documents[i].szClass + "      | ";
            for (int x = 0; x < featureCount; x++)
            {
                out += integerToFixedSizeString(documents[i].iFeatures[x],2) + " | ";
            }
            out += documents[i].szText;
            //Print it
...
public class Main {
    ...
    public static void main(String[] args)
    {
        ...
        KNearestNeighbors KNN = new KNearestNeighbors(features, 10);
        ...
        System.out.println("Calculations took " + Integer.toString(msPassed) + " Nanoseconds");
    }
}
